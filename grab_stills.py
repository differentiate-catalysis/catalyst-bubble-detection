...
parser = ArgumentParser(description="Utility script to grab still images from videos, labeling them with timestamps "
...
cam = cv2.VideoCapture(args.source_video_path)
# frame counter
currentframe = 0
...
n_frame_to_grab = args.save_rate * math.ceil(fps)
...
while True:
    ...
    if currentframe % n_frame_to_grab == 0.0:
        ...
        if ret:
            filename = f"{args.video_name}_{str(timestamp)}.jpg"
