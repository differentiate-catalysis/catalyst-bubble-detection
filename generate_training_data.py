...
def extract_bubbles(image_root, json_root, output_dir):
    i = 0
...
def augment_bubble(bubble: Image.Image):
    bubble_tensor = pil_to_tensor(bubble)
    transform = T.Compose([
        T.ColorJitter(0.25, 0.25, 0.25),
        T.RandomApply([T.GaussianBlur(3, 0.1)]),
        T.RandomAdjustSharpness(0.2),
    ])
    bubble_tensor[:3, :, :] = transform(bubble_tensor[:3, :, :])
    bubble_tensor = bubble_tensor.permute((1, 2, 0)).numpy()
    bubble = Image.fromarray(bubble_tensor)
    return bubble
...
def generate_data(root: str, num_samples: int, min_bubble: int, max_bubble: int, width_padding: int = 200, height_padding: int = 200):
    ...
    for sample in range(num_samples):
        ...
        for _ in range(num_bubbles):
            ...
            bubble_image = augment_bubble(bubble_image)
        ...
        background.save(os.path.join(output_dir, '%d.png' % sample))


if __name__ == '__main__':
    generate_data('data/data_generation/pristine', 5, 10, 50)
